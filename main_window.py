...
MODEL_PATH = os.path.join(_BUNDLE, 'models', MODEL_NAME + '.onnx')
if not os.path.exists(MODEL_PATH):
    MODEL_PATH = os.path.join(APP_DIR, 'models', MODEL_NAME + '.onnx')
...
def _process(task, video_mode, report):
    ...
    try:
        ...
        report('③ 加载并分离伴奏 / 人声（CPU 推理，请耐心等待）…', 10)
        sep = get_separator()
        with _JOBS_LOCK:
            inst = sep.demix(mix, progress=lambda i, t: report(
                '③ 分离伴奏 / 人声…', 10 + (0 if t <= 0 else int(i * 90 / t))))
...
class MainFrame(wx.Frame):
    ...
    def _build_ui(self):
        ...
        title.Add(wx.StaticText(self.panel, label='伴奏分离'), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10)
        badge = wx.StaticText(self.panel, label='Inst HQ 3', style=wx.ALIGN_CENTRE_HORIZONTAL)
        badge.SetForegroundColour(wx.Colour(255, 255, 255))
        badge.SetBackgroundColour(wx.Colour(51, 122, 183))
        badge.SetMinSize((74, 24))
        title.Add(badge, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 6)
        ver = wx.StaticText(self.panel, label='v1.3.0', style=wx.ALIGN_CENTRE_HORIZONTAL)
        ...
        sub = wx.StaticText(self.panel,
                            label='基于 UVR-MDX-NET-Inst_HQ_3 模型，本地 CPU 处理。可一次拖入 / 选择多个音视频，自动排队依次处理。',
        ...
        self.chk_shutdown = wx.CheckBox(self.panel, label='所有任务处理完成后自动关机')
        root.Add(self.chk_shutdown, 0, wx.LEFT | wx.TOP, 8)
    ...
    def _work_loop(self):
        while not self._stop:
            ...
            _process(task, video_mode, self._make_reporter(task))
